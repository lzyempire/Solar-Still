...
Solar_climate <- read.csv(file = "CR1000_BSRN1000_Min201009.csv", skip = 1, stringsAsFactors = FALSE)
...
cleandata <- function(SolarWater){
  hourlywater <- rbind(rbind(c("0:00", 0), na.omit(SolarWater[2:length(SolarWater[, 1]), 1:2]), c("23:59", SolarWater[1, 2])))
  names(hourlywater) <- c("Time", "WaterProduction")
  hourlywater <- cbind(Date = SolarWater[1, 1], hourlywater)
  hourlywater$Percentage <- as.numeric(hourlywater$WaterProduction)*100/as.numeric(SolarWater[1, 2])
  for(i in 2:(length(SolarWater)/2)){
    hourdata <- rbind(rbind(c("0:00", 0), na.omit(SolarWater[2:length(SolarWater[, (2*i-1)]), (2*i-1):(2*i)]), c("23:59", SolarWater[1, (2*i)])))
    names(hourdata) <- c("Time", "WaterProduction")
    hourdata <- cbind(Date = SolarWater[1, (2*i-1)], hourdata)
    hourdata$Percentage <- as.numeric(hourdata$WaterProduction)*100/as.numeric(SolarWater[1, (2*i)])
    hourlywater <- rbind(hourlywater, hourdata)
  }
  hourlywater <- hourlywater[hourlywater$Date >= data_record, ]
  hourlywater$Date <- ymd(hourlywater$Date)
...
meltcol <- function(colnames){
  mergecolselect <- rbind(rbind(rbind(rbind(cbind(hourlywater_BottomInter[, colnames], Still = "Bottom_Interfacial"), cbind(hourlywater_SidewallInter[, colnames], Still = "Sidewall_Interfacial")), cbind(hourlywater_FoamBottomInter[, colnames], Still = "Foam_Bottom_Interfacial")), cbind(hourlywater_SidewallHeat[, colnames], Still = "Sidewall_Bottom_Heating")), cbind(hourlywater_BottomHeat[, colnames], Still = "Bottom_Heating"))
...
StillName_Interfacial <- c("Bottom_Interfacial", "Foam_Bottom_Interfacial", "Sidewall_Interfacial")
...
TransEff + geom_point(alpha = 0.2) + geom_smooth(se = FALSE) + labs(x = "Time", y = "Hourly Transient Efficiency/%") + scale_y_continuous(limits = c(10,75)) + scale_x_continuous(limits = c(7,20))
...
PowerEff + geom_point(alpha = 0.2) + geom_smooth(se = FALSE) + labs(x = "Solar Power/W", y = "Hourly Transient Efficiency/%") + scale_y_continuous(limits = c(10,75)) + scale_x_continuous(limits = c(100,1000))
