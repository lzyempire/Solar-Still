...
library(lubridate)
data_record <- as.Date("2020-08-17")
SolarWater <- read.csv(file = "Sidewall Bottom Heating Solar Still Water Production.csv", header = FALSE, stringsAsFactors = FALSE)
...
fit2 <- lm(Global_Efficiency ~ Direct_Energy_Tot + Diffuse_Energy_Tot, data = na.omit(SolarData_Day[-c(21, 33, 34), ]))
...
write.csv(SolarData_Day, file = "Sidewall Bottom Heating Solar Still Daily Water Production.csv")
